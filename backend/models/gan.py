...
class GANModel:
            ...
            for j, v in enumerate(nums_inv[i]):
                if np.isnan(v):
                    ...
                    if self.pre.num_cols[j] in ("ApplicantIncome", "CoapplicantIncome"):
                        val = int(max(self.min_income, min(self.max_income, round(v))))
                    elif self.pre.num_cols[j] == "LoanAmount":
                        val = int(max(self.min_loan, min(self.max_loan, round(v))))
    ...
    def sample(self, n):
        num_dim = len(self.pre.num_cols)
        cat_dim = len(self.pre.cat_cols)
        nums = self.rng.normal(loc=0.0, scale=1.0, size=(n, num_dim)) if num_dim > 0 else np.zeros((n, 0))
        ...
        for j in range(cat_dim):
            ...
            cats[:, j] = self.rng.randint(0, k, size=n)
